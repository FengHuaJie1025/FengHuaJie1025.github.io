# 个人展示网站 + AI 知识库 — 需求与技术方案（PRD）

> 版本：v1.0 ｜ 2026-08-01 ｜ 状态：需求已收敛，分两期实施（一期展示站 / 二期 AI 查询）
...
| 部署 | 腾讯云 **EdgeOne Pages**（国内边缘节点，≈免费，默认子域免备案） |
...
- **项目经历** `src/content/projects/*.md`
  - frontmatter：`title` / `role` / `stack[]` / `period` / `summary` / `highlights[]` / `links[{label,url}]` / `cover?` / `order` / `draft`
  - body：详细描述（Markdown）
...
│  │  ├─ projects/   (项目经历 .md)
│  │  └─ notes/      (知识沉淀 .md)
│  ├─ components/    (Header / Footer / ProjectCard / NoteCard / ThemeToggle)
│  ├─ layouts/       (BaseLayout)
│  ├─ pages/         (首页 / projects / notes / search / about)
│  └─ styles/        (global.css)
...
## 7. 部署（EdgeOne Pages）
- 关联 Git 仓库（GitHub/GitLab/Gitee），push 自动构建部署
- 默认子域名（`*.edgeone.app`）先跑，免备案
- 自定义域名：后续买域名 + ICP 备案后绑定
...
3. 展示页（首页/项目/知识/详情）✅（一期）
4. 全文搜索（Pagefind）✅（一期）
5. 暗色模式 / 响应式 ✅（一期）
6. 关联 EdgeOne Pages，部署验证 ✅（一期）
7. 【二期】构建脚本生成向量索引
